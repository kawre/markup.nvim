...
function Text:draw(body, parent)
    local len = self:length()
    local row, col = body:row(), body:col(len)

    local row_start = row - 1
    local col_start = col

    local content = self:content()
    vim.api.nvim_buf_set_text(body.bufnr, row_start, col_start, row_start, col_start, { content })
    self:highlight(body.bufnr, -1, row, col)
end
